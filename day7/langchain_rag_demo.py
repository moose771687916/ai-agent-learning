# ============================================================
# langchain_rag_demo.py - LangChain RAG + 持久化 + 记忆
# 启动：python -m uvicorn langchain_rag_demo:app --reload
...
app = FastAPI(title="LangChain RAG演示")
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        if not text.strip():
            return {"error": "文件内容是空的，可能是扫描版PDF（图片转的），读不出来"}
        ...
        <script>
            ...
            async function upload() {
                try {
                    ...
                    alert("开始上传：" + fileInput.files[0].name);
